...
class TestDhcpStaticFlowVerification:
    """DHCP静态分配功能验证(L5): edit既有绑定IP→DHCP重获验新IP→恢复原IP.

    测试前用客户端网关邻居MAC与目标路由器LAN MAC做拓扑防错，避免Web/SSH测A设备、
    DHCP客户端却接在B设备。客户端接口动态识别；重获使用networkctl down/up，必须观察到
    旧地址撤销、当前接口租约文件重建和DHCP路由，不能把网卡上残留的旧IP当成新租约。
    client MAC已有绑定时编辑实际命中的规则，测完恢复原IP，环境最终不变。"""
    ...
    def test_dhcp_static_flow(self, dhcp_static_page_logged_in: DhcpStaticPage,
        ...
        try:
            ...
            # 步骤2: 动态识别client接口，并确认它确实接在当前目标路由器上。
            with rec.step("步骤2: 获取client并校验拓扑", "动态识别接口/MAC，网关MAC须等于目标路由器LAN MAC"):
                info = bv.get_client_lan_info(self.GATEWAY)
                client_iface = info.get("iface")
                client_mac = info.get("mac")
                rec.add_detail(f"  client: iface={client_iface} ip={info.get('ip')} mac={client_mac}")
                if not client_iface or not client_mac:
                    pytest.fail("测试环境配置错误: 未获取到client接口或MAC")
                topology = bv.verify_client_gateway_matches_router(
                    self.GATEWAY,
                    client_iface=client_iface,
                )
                rec.add_detail(f"  {topology.level}: {'[OK]' if topology.passed else '[FAIL]'} {topology.message}")
                router_lan_iface = topology.details.get("router_iface")
                if not topology.passed:
                    pytest.fail(topology.message)

            # 步骤3: edit既有绑定的IP(client MAC环境已有lan1→固定IP绑定, 新建auto会被lan1精确接口覆盖;
            #        故edit实际命中的既有绑定→STATIC_IP, DHCP重获后验新IP, 测完恢复原IP)
            with rec.step("步骤3: edit既有绑定IP+L1-L3验下发",
                ...
                existing = [s for s in all_static
                            ...
                            and str(s.get("enabled", "")).lower() == "yes"
                            and str(s.get("interface", "")).lower()
                            in {"auto", str(router_lan_iface or "").lower()}]
                if not existing:
                    pytest.fail(
                        f"测试环境配置错误: client MAC {client_mac} 在"
                        f"{router_lan_iface or '目标LAN'}无启用的静态绑定"
                    )
                # 与服务端匹配优先级一致：精确接口规则优先于auto规则。
                existing.sort(
                    key=lambda row: 0 if str(row.get("interface", "")).lower()
                    == str(router_lan_iface or "").lower() else 1
                )
                orig = existing[0]
                ...
                conflicts = [
                    row for row in all_static
                    if row.get("ip_addr") == self.STATIC_IP
                    and (row.get("mac", "") or "").lower() != client_mac.lower()
                ]
                target_lease = bv.query_lease(ip=self.STATIC_IP)
                lease_mac = str((target_lease or {}).get("mac", "")).lower()
                if conflicts or (target_lease and lease_mac != client_mac.lower()):
                    pytest.fail(f"测试环境配置错误: 目标IP {self.STATIC_IP} 已被其他终端或绑定占用")
                ...
                # 不主动restart服务。必须由UI保存触发产品自身的delayed_restart并完成下发，
                # 否则强制重启会掩盖“配置已入库但运行时未生效”的真实缺陷。
                chain = None
                for _ in range(15):
                    chain = bv.verify_dhcp_static_full_chain(
                        name=orig_tagname,
                        expected_fields={
                            "enabled": "yes",
                            "interface": orig.get("interface"),
            ...
            # 步骤4: 必须先看到旧地址被撤销，再验客户端和路由器租约都为绑定IP。
            with rec.step("步骤4: DHCP重获验绑定生效",
                          f"{client_iface}清旧租约并重获→client/路由器租约均应为{self.STATIC_IP}(硬)"):
                renew = bv.renew_client_dhcp(client_iface, expected_ip=self.STATIC_IP, timeout=35)
                new_ip = renew.details.get("new_ip", "")
                rec.add_detail(f"  {renew.level}: {'[OK]' if renew.passed else '[FAIL]'} {renew.message}")
                rec.add_detail(
                    f"  地址迁移: old={renew.details.get('old_ips')} cleared="
                    f"{renew.details.get('address_cleared')} new={new_ip} "
                    f"client_lease={renew.details.get('lease_ip')}"
                )
                if not renew.passed:
                    failures.append(f"绑定IP未生效: {renew.message}")

                router_lease = None
                for _ in range(8):
                    router_lease = bv.query_lease(mac=client_mac)
                    if router_lease and router_lease.get("ip_addr") == self.STATIC_IP:
                        break
                    page.page.wait_for_timeout(1000)
                lease_ok = bool(
                    router_lease
                    and router_lease.get("ip_addr") == self.STATIC_IP
                    and str(router_lease.get("mac", "")).lower() == client_mac.lower()
                )
                rec.add_detail(f"  路由器租约: {router_lease or '无'}")
                if not lease_ok:
                    failures.append(
                        f"路由器租约未同步: 实际={router_lease or '无'}, 期望IP={self.STATIC_IP}"
                    )

            # 步骤5: 恢复既有绑定IP(orig_ip) → DHCP重获并硬验证恢复结果。
            with rec.step("步骤5: 恢复既有绑定IP", f"edit {orig_tagname} IP→{orig_ip}(恢复环境) + DHCP重获"):
                page.navigate_to_dhcp_static()
                page.page.wait_for_timeout(800)
                try:
                    restore_ok = page.edit_dhcp_static(orig_tagname, ip=orig_ip)
                    rec.add_detail(f"  [UI] 恢复 {orig_tagname} IP→{orig_ip}")
                    if not restore_ok:
                        failures.append("恢复既有绑定失败: UI返回失败")
                except Exception as e:
                    rec.add_detail(f"  ✗ 恢复edit失败: {str(e)[:60]}")
                    failures.append(f"恢复既有绑定失败: {str(e)[:60]}")
                restore_chain = None
                for _ in range(15):
                    restore_chain = bv.verify_dhcp_static_full_chain(
                        name=orig_tagname,
                        expected_fields={
                            "enabled": "yes",
                            "interface": orig.get("interface"),
                    ...
                    if restore_chain.all_passed:
                        break
                    page.page.wait_for_timeout(1000)
                for result in restore_chain.results:
                    rec.add_detail(
                        f"  恢复-{result.level}: "
                        f"{'[OK]' if result.passed else '[FAIL]'} {result.message}"
                    )
                    if not result.passed:
                        failures.append(f"恢复-{result.level}: {result.message}")
                restore = bv.renew_client_dhcp(client_iface, expected_ip=orig_ip, timeout=35)
                restore_ip = restore.details.get("new_ip", "")
                rec.add_detail(f"  {restore.level}: {'[OK]' if restore.passed else '[FAIL]'} {restore.message}")
                if not restore.passed:
                    failures.append(f"环境恢复验证失败: client IP={restore_ip or '无'}, 期望={orig_ip}")
        finally:
            ...
            try:
                ...
                pass
            if client_iface and orig_ip:
                try:
                    bv.renew_client_dhcp(client_iface, expected_ip=orig_ip, timeout=35)
                except Exception:
                    pass
