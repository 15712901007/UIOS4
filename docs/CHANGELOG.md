...
### 根因
跨三层服务选中行后底部会出现批量操作栏(div.footer含"已选X条"+"启用/停用/删除"),
但_click_batch_button没等待它出现就去查找,导致找不到工具栏按钮,误以为页面没有批量删除。

### 修复
- _click_batch_button开头等待"已选X条"文字出现(最多3秒),确保底部操作栏已渲染
- 跨三层步骤1/16/18/21恢复使用batch_delete(不再逐行删除)
- 验证: 1 passed in 349s (步骤16批量删除成功+SSH验证通过)

## 2026-06-17 跨三层服务虚拟滚动表格适配(逐行删除)

### 根因: 跨三层用ant-design虚拟滚动表格
- 页面无工具栏批量删除/停用/启用按钮(全选后点行内删除只删当前行)
- get_rule_list只返回可见行(DOM只渲染当前视口的行), 返回空列表
- 导致逐行删除读到0条, 一个都没删, SSH验证发现8条未删除

### 修复
- 步骤16: 先尝试页面get_rule_list,为空则用SSH(query_netsnmpc_rules)获取完整列表
- 步骤1/16/18/21的batch_delete()全部改为逐行delete_rule()
...
### 问题: 测试失败被判通过(严重)
- **现象**: 跨三层步骤16批量删除后SSH验证发现8条规则未删除, 但步骤判"通过"
- **根因**: SSH验证失败只print WARNING不加入ui_failures, 被吞掉了
- **修复**: 加ui_failures.append, SSH验证失败时判FAILED
- **全模块排查**: 扫描35处WARN, 其中批量删除SSH验证只有跨层这一处被吞(已修),
  其余为帮助图标/排序/搜索等辅助功能(不阻断合理)

### 验证结果
- 跨三层: 1 failed (正确! 批量删除SSH验证发现8条未删除, 不再吞掉)
- 根因是_click_batch_button在跨三层页面找不到工具栏批量按钮(UI交互问题, 非框架问题)
...
- **根因**: `_click_batch_button`方法1用`get_by_role("button", name="delete 删除").first`,
  页面有**两个**同名按钮(行内删除 + 工具栏批量删除都有delete图标), .first匹配到行内的,
  导致全选后只删了1条而非批量删除
- **修复**: 方法1加`ancestor::tr`检查, 跳过行内按钮, 只点工具栏的
- **影响**: 此修复改善了**所有模块**的批量操作可靠性(批量停用/启用/删除)
- **验证**: 1 passed in 298s (步骤15批量停用9/9 + 步骤17批量删除SSH验证通过)

### 跨三层服务all_failures赋值修复(漏网之鱼)
- **根因**: 之前的批量修复脚本对跨三层服务的断言格式没匹配上(有"SSH失败:"等特殊格式),
  导致all_failures赋值仍在if ssh_failures:块内
- **修复**: 手动把赋值移到if之前

### 本轮全量测试3个失败最终状态
| 模块 | 原因 | 状态 |
|------|------|------|
| IP限速 | _click_batch_button点到行内按钮 | ✅ 已修复(1 passed) |
| 跨三层服务 | all_failures赋值位置 | ✅ 已修复 |
| DMZ主机 | 产品bug(netmap.sh select*) | ✅ 正确判FAILED |
...
### 导入导出覆盖
- **导出**: CSV + TXT 两种格式
- **导入**: 步骤16 CSV追加导入 / 步骤17 TXT清空现有后导入
...
### 测试覆盖全景
- **负载模式**: 新建连接数(0) / 源IP(1) / 源IP+源端口(2) / 源IP+目的IP(3) / 源IP+目的IP+目的端口(4) / 主备模式(6) — 6种全覆盖
- **分流方式**: 外网线路(type=0) + 下一跳网关(type=1) — 2种全覆盖
- **协议**: any / tcp / udp / tcp+udp / icmp — 5种全覆盖
- **扩展功能**: 线路绑定(iface_band) / 生效时间(time) / 源地址反向匹配(src_addr_inv) / IP分组引用(src_addr.object) / 源端口+目的端口
