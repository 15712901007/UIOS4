...
class IpRateLimitPage(BasePage):
    """IP限速页面操作类"""
    ...
    def select_line(self, line: str = "任意"):
        ...
        try:
            ...
            if line_combobox.count() > 0:
                ...
                # 点击对应的label选项（选项是checkbox在label中）
    ...
    def add_ip_address(self, ip: str):
        ...
        Args:
            ip: IP地址（支持单个IP、IP段如192.168.1.1-192.168.1.100、CIDR如192.168.0.0/24）

        Note:
            UI交互方式：点击"添加"按钮会新增一个输入行，输入IP后无需点击确认，
            IP会自动保存在输入框中。每个IP占一行。
        """
        try:
            # 点击IP设置区域的"添加"按钮
            # 注意：页面上有多个"添加"按钮（顶部导航、IP设置区域），需要使用.last精确定位
            # 参考VLAN页面的扩展IP实现
            ...
            # 填写IP地址 - 使用.last定位最新添加的输入框
            # 因为每次点击"添加"都会新增一个输入行，所以要用.last获取最新的
            # 注意：placeholder使用中文全角括号（和）和中文弯引号（"和"）
    ...
    def add_cidr(self, cidr: str):
        ...
        Args:
            cidr: CIDR格式IP地址（如192.168.0.0/24）
        """
        return self.add_ip_address(cidr, "CIDR")
    ...
    def batch_add_ips(self, ips: List[str]):
        ...
        Args:
            ips: IP地址列表（如 ["192.168.1.1", "192.168.1.2", "192.168.1.0/24"]）

        Note:
            UI交互：点击"批量"按钮 → 弹出对话框 → 输入多个IP（一行一条） → 点击确定
            支持格式：单个IP(192.168.1.1) 或 CIDR(192.168.1.0/24)
        """
        try:
            # 点击批量按钮
            ...
            # 输入多个IP（一行一条，用换行符分隔）
            ip_text = "\n".join(ips)
            # 使用exact=True精确匹配placeholder="请输入"，避免匹配到其他包含"请输入"的元素
    ...
    def create_ip_group_in_dialog(self, group_name: str, ip_list: str):
        ...
        Args:
            group_name: 分组名称
            ip_list: IP列表（如192.168.10.1-192.168.10.100）
        """
        try:
            # 点击IP分组下拉框
            ip_group_combobox = self.page.locator(".ant-select-selector").nth(1)  # IP分组是第二个下拉框
    ...
    def create_time_plan_in_dialog(self, plan_name: str, weekdays: list = None, start_time: str = "09:00", end_time: str = "18:00"):
        """
        在添加弹窗内创建时间计划

        Args:
            plan_name: 时间计划名称
            weekdays: 生效日期列表（如["一", "二", "三", "四", "五"]），默认周一至周五
            start_time: 开始时间，默认09:00
            end_time: 结束时间，默认18:00
        """
        try:
            # 先选择"时间计划"单选按钮
    ...
    def select_protocol(self, protocol: str = "tcp"):
        ...
        Args:
            protocol: 协议类型（tcp/udp/tcp+udp/任意）
        """
        try:
            # 协议字段名称带 * 号
    ...
    def set_time_by_week(self, days: List[str] = None, start_time: str = "00:00", end_time: str = "23:59"):
        ...
        """
        try:
            # 关闭可能存在的 Ant Design 模态对话框（如 IP 分组选择对话框）
    ...
    def add_rule(self, name: str,
        ...
        Args:
            name: 规则名称
            line: 线路（全部/wan1/wan2/wan3/任意）
            ip: IP地址（支持单个IP、IP段、CIDR格式）
            ip_group: IP分组名称（可选）
            protocol: 协议（tcp/udp/tcp+udp/任意）
            rate_mode: 限速模式（独立限速/共享限速）
            upload_speed: 上行限速值
    ...
    def try_add_rule_invalid(self, name: str = "", remark: str = "", ip: str = "",
        ...
        try:
            ...
            if expect_fail and name:
                ...
                # 使用精确匹配检查规则是否被意外创建
                # 使用XPath在表格行中查找名称列精确匹配的行（避免匹配到IP分组名称等其他列）
                try:
                    # 使用XPath查找表格行中第一列（名称列）包含精确规则名称的行
                    # 这样可以避免匹配到IP分组名称（如test_ip_group_001）
    ...
    def select_rule(self, rule_name: str, timeout: int = 10000):
        ...
        try:
            ...
            # 使用exact=True精确匹配规则名称，设置合理的超时
            rule_text = self.page.get_by_text(rule_name, exact=True)
            # 先等待元素出现
            rule_text.wait_for(timeout=timeout)

            # 使用evaluate找到并点击同一行的复选框（与VLAN页面相同的方法）
    ...
    def sort_by_column(self, column_name: str):
        ...
        Args:
            column_name: 列名（线路/限速模式/上行限速/下行限速）
        """
        try:
    ...
    def test_sorting(self) -> dict:
        ...
        for col in columns:
            try:
                ...
                self.page.wait_for_timeout(300)
                self.sort_by_column(col)  # 再次点击切换排序方向
                self.page.wait_for_timeout(300)
                result[col] = True
            except Exception:
                result[col] = False
    ...
    def export_rules(self, use_config_path: bool = True, export_format: str = "csv") -> bool:
        ...
        try:
            ...
            if confirm_btn.count() > 0 and confirm_btn.is_visible():
                ...
                if use_config_path:
                    config = get_config()
                    base_path = config.test_data.get_export_path("ip_rate_limit", config.get_project_root())
                    save_path = os.path.splitext(base_path)[0] + f".{export_format.lower()}"
                else:
                    download_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "downloads")
                    os.makedirs(download_dir, exist_ok=True)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    save_path = os.path.join(download_dir, f"ip_rate_limit_export_{timestamp}{original_ext}")
